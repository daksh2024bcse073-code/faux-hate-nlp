...
# =========================================
# E5 EMBEDDER (IMPORTANT: uses "passage:" prefix)
# =========================================
...
class E5Embedder:
    ...
    def encode(self, texts, batch_size=128):
        ...
        for i in tqdm(range(0, len(texts), batch_size)):
            ...
            # E5 needs prefix
            batch = ["passage: " + str(t) for t in batch]
...
def train_e5_mistral():

    print("="*60)
    print("FAST E5 + Retrieval Pipeline")
    print("="*60)
    ...
    train_emb = get_embeddings(embedder, train_df["text"].astype(str).tolist(), "e5_train.npy")
    val_emb   = get_embeddings(embedder, val_df["text"].astype(str).tolist(), "e5_val.npy")
    test_emb  = get_embeddings(embedder, test_df["text"].astype(str).tolist(), "e5_test.npy")
    ...
    with open("results/rag/e5_mistral_results.json", "w") as f:
        json.dump(results, f, indent=4)

    print("\nSaved → results/rag/e5_mistral_results.json")
